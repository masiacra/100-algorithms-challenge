...
book is that you can number given the current page and numberOfDigits 
left. A page is considered numbered if it has the full number printed 
on it (e.g. if we are working with page 102 but have ink only for two 
digits then this page will not be considered numbered).
...
A positive integer, the number on the current page which is not yet 
printed.
...
function f(current, numberOfDigits) {
	let number = current.toString().length;
	let aux = current + 1;
	 while (true) {
		current++;
		number += current.toString().length;
		if (number > numberOfDigits) {
			current--;
			break;
		}
	} 
