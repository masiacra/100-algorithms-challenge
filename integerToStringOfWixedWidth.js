...
the given number to have a specified length. We are allowed to do that 
either by cutting out leading digits (if the number needs to be shortened) 
...
function f(number, width) {
	number += '';
	if (width < number.length) {
		return number.slice(number.length - width);
	} else if (width === number.length) {
		return number;
	} else {
		return number = '0'.repeat(width - number.length) + number;
	}
}
