...
cost_per_minute[i] is a positive number denoting cost per minute for the 
ith car type. There are at least 4 car types in every city where Uber 
operates.
...
cost_per_mile[i] is a positive number denoting cost per mile for the ith 
car type. It is guaranteed that cost_per_minute and cost_per_mile have 
...
function f(ride_time, ride_distance, cost_per_minute, cost_per_mile) {
	let result = [];
	cost_per_minute.forEach( (cost, ind) => {
		let val = Math.round( (ride_time * cost + cost_per_mile[ind] * ride_distance) *
			10) / 10;
		result.push( val );
	});
	return result;
